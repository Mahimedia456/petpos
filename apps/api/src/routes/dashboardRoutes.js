...
router.get("/summary", authMiddleware, async (req, res) => {
  try {
    const [
      ...
      pool.query(`SELECT COUNT(*)::int AS count FROM products WHERE stock_qty <= low_stock_threshold`),
      ...
      `),
      pool.query(`SELECT COUNT(*)::int AS count FROM orders WHERE status IN ('pending', 'processing')`),
    ]);
